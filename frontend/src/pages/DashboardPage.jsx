...
function DashboardPage({ setHasSmtp }) {
    ...
    const fetchStats = async () => {
      try {
        const [sent, drafts, scheduled, groups] = await Promise.all([
          axios.get('http://192.168.1.6:5000/emails?status=sent'),
          axios.get('http://192.168.1.6:5000/emails?status=draft'),
          axios.get('http://192.168.1.6:5000/emails?status=scheduled'),
          axios.get('http://192.168.1.6:5000/groups'),
        ]);
        setStats({
  ...
  const handleSaveSMTP = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('http://192.168.1.6:5000/auth/smtp', smtpData, {
